...
class Billing extends Controller
{
    ...
    function schetfactura()
    {
        ...
        if (!empty($_POST['rec_name'])) {
            $data['rec_name'] = $_POST['rec_name'];
        }
        if (!empty($_POST['rec_bin'])) {
            $data['rec_bin'] = $_POST['rec_bin'];
        }
        if (!empty($_POST['rec_add'])) {
            $data['rec_add'] = $_POST['rec_add'];
        }
        if (!empty($_POST['rec_iik'])) {
            $data['rec_iik'] = $_POST['rec_iik'];
        }
        if (!empty($_POST['rec_bank'])) {
            $data['rec_bank'] = $_POST['rec_bank'];
        }
        if (!empty($_POST['rec_bank_bik'])) {
            $data['rec_bank_bik'] = $_POST['rec_bank_bik'];
        }

        if (!empty($_POST['cons_bin'])) {
            $data['cons_bin'] = $_POST['cons_bin'];
        }
        if (!empty($_POST['cons_add'])) {
            $data['cons_add'] = $_POST['cons_add'];
        }
        if (!empty($_POST['cons_iik'])) {
            $data['cons_iik'] = $_POST['cons_iik'];
        }
        if (!empty($_POST['cons_bank'])) {
            $data['cons_bank'] = $_POST['cons_bank'];
        }
        if (!empty($_POST['cons_bank_bik'])) {
            $data['cons_bank_bik'] = $_POST['cons_bank_bik'];
        }
        #end of added


        #added
        if (!empty($_POST['shet2'])) {
            $schet_id = (int)$_POST['shet2'];

            $this->db->where("id", $_POST['period_id']);
            $schet_year = get_year_number($this->db->get("industry.period")->row()->begin_date);
            $this->db->where("schet_year", $schet_year);
            $this->db->where('period_firm <> ', $_POST['period_id'].$_POST['firm_id']);
            $this->db->where("schet_id", $schet_id);
            $dup_schet = $this->db->get("industry.existing_schet_id");
            if($dup_schet->num_rows()){
                header('Content-Type: text/html; charset="utf-8"');
                $dup_schet = $dup_schet->row();
        ...
        #added
        if (!empty($_POST['schet_nakl'])) {
            $this->db->where('period_id', $_POST['period_id']);
            $this->db->where('firm_id', $_POST['firm_id']);
            $this->db->update('industry.schetfactura_date', array(
                'schet_id' => (int)$_POST['schet_nakl']
            ));
        }
        #end of added
        if (!isset($_POST['html'])) {
...
where firm_id={$this->uri->segment(3)} and data_finish is null";
    ...
    public function export_rekvizit_schet()
    {
        ...
        if ($db) {
            ...
            if (!copy("c:/oplata/rschet.dbf", "c:/oplata/zelina{$month}.dbf")) {
